...
import { buildGraph, layoutGraph, renderGraph, enableGraphNavigation } from './heartline-v301-graph.js';
...
async function renderGraphRoute() {
  ...
  view.className = 'view';
  view.innerHTML = `<section class="page full"><div class="graph-page"><div class="graph-toolbar"><input id="graphSearch" class="input" placeholder="Сцена, Choice ID или название" value="${Domain.escapeHtml(state.graphSearch)}"><select id="graphFilter" class="select"><option value="all">Вся новелла</option><option value="unread">Непройденные</option><option value="missing">Без изображений</option><option value="reviews">С замечаниями</option><option value="endings">Концовки</option></select><div class="inline-actions"><button id="graphZoomOut" class="button secondary small">−</button><span id="graphZoomLabel" class="status-badge">75%</span><button id="graphZoomIn" class="button secondary small">+</button><button id="graphFit" class="button secondary small">Вместить</button></div></div><div class="graph-body"><div id="graphViewport" class="graph-viewport" tabindex="0"><div id="graphCanvas" class="graph-canvas"></div></div><aside id="graphInspector" class="graph-inspector"><span class="kicker">STORY GRAPH</span><h2>Карта ветвлений</h2><p class="muted">Колесо — масштаб. Средняя кнопка или Space+drag — перемещение. Двойной клик открывает сцену.</p></aside></div></div></section>`;
  $('graphFilter').value = state.graphFilter;
  const draw = () => {
    const svg = renderGraph($('graphCanvas'), model, layout, { currentSceneId: effectiveFrame()?.sceneId, visitedSceneIds: visited, filter: state.graphFilter, search: state.graphSearch, onSelect: node => showGraphInspector(node), onOpen: node => { const first = sceneFrames(sceneById(node.sceneId))[0]; if (first) selectFragment(first.fragmentId, { route: 'reader', direct: true }); } });
    state.graphNavigation = enableGraphNavigation($('graphViewport'), svg, { initial: .75, onZoom: zoom => { if ($('graphZoomLabel')) $('graphZoomLabel').textContent = `${Math.round(zoom * 100)}%`; } });
  };
  draw();
...
function showGraphInspector(node) {
  ...
  if (!host) return;
  if (node.kind === 'choice') {
    host.innerHTML = `<span class="kicker">CHOICE</span><h2>${Domain.escapeHtml(node.title)}</h2><p class="muted">${Domain.escapeHtml(node.choiceId)}</p><button id="graphReader" class="button primary">Открыть сцену</button>`;
  } else {
    host.innerHTML = `<span class="kicker">СЦЕНА</span><h2>${Domain.escapeHtml(node.title)}</h2><p class="muted">${Domain.escapeHtml(node.sceneId)} · ${Domain.escapeHtml(node.chapterTitle)}</p><div class="metric-grid" style="grid-template-columns:repeat(2,1fr)"><div class="metric"><strong>${node.metrics.frames}</strong><span>кадров</span></div><div class="metric"><strong>${node.metrics.missing}</strong><span>без изображений</span></div><div class="metric"><strong>${node.metrics.approved}</strong><span>утверждено</span></div><div class="metric"><strong>${node.metrics.reviews}</strong><span>замечаний</span></div></div><div class="grid"><button id="graphReader" class="button primary">Открыть в Reader</button><button id="graphStoryboard" class="button secondary">Открыть Storyboard</button><button id="graphPreview" class="button secondary">Открыть Preview</button></div>`;
  }
