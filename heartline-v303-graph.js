...
function structureGeometry(model, visible) {
  const chapterOrder = model.chapterOrder.filter(ch => model.nodes.some(n => visible.has(n.id) && n.chapterTitle === ch));
  const routes = ['common', 'equal', 'fire', 'mask', 'direct'].filter(key => model.nodes.some(n => visible.has(n.id) && (n.routeKey || 'common') === key));
  ...
  routes.forEach((key, i) => laneMeta.set(key, { y: top + i * laneHeight, label: ROUTE_META[key].label }));
  chapterOrder.forEach((chapter, ci) => {
    const x = left + ci * chapterWidth;
    chapterMeta.push({ chapter, x, width: chapterWidth - 18 });
    for (const key of routes) {
      const items = model.nodes.filter(n => visible.has(n.id) && n.chapterTitle === chapter && (n.routeKey || 'common') === key).sort((a, b) => a.order - b.order);
      items.forEach((node, i) => positions.set(node.id, { x: x + 18, y: laneMeta.get(key).y + 34 + i * 82, width: node.kind === 'choice' ? 276 : 250, height: node.kind === 'choice' ? 66 : 60 }));
    }
  });
  const maxY = Math.max(...[...positions.values()].map(p => p.y + p.height), 600);
  return { positions, chapterMeta, laneMeta, routes, width: Math.max(1200, left + chapterOrder.length * chapterWidth + 40), height: Math.max(650, maxY + 70) };
}

function edgePath(from, to) {
  const x1 = from.x + from.width, y1 = from.y + from.height / 2;
  const x2 = to.x, y2 = to.y + to.height / 2;
  const dx = Math.max(48, (x2 - x1) * .46);
...
function renderStructure(host, model, options) {
  ...
  const matched = new Set(model.nodes.filter(n => q && nodeSearchText(n).includes(q)).map(n => n.id));
  const svg = document.createElementNS('http://www.w3.org/2000/svg', 'svg');
  svg.setAttribute('viewBox', `0 0 ${geo.width} ${geo.height}`); svg.setAttribute('width', geo.width); svg.setAttribute('height', geo.height);
  svg.classList.add('story-graph-svg', 'graph-structure-view');
  const defs = svgEl(svg, 'defs'); defs.innerHTML = '<marker id="graph-arrow" markerWidth="8" markerHeight="8" refX="7" refY="3" orient="auto" markerUnits="strokeWidth"><path d="M0,0 L0,6 L8,3 z" fill="currentColor"/></marker>'; svg.append(defs);

  const bg = svgEl(svg, 'g', { class: 'graph-background' });
  geo.chapterMeta.forEach((c, i) => { bg.append(svgEl(svg, 'rect', { x: c.x - 8, y: 28, width: c.width, height: geo.height - 54, rx: 16, class: `graph-chapter-band ${i % 2 ? 'alt' : ''}` })); const t = svgEl(svg, 'text', { x: c.x + 12, y: 54, class: 'graph-chapter-title' }); t.textContent = c.chapter; bg.append(t); });
  for (const key of geo.routes) { const lane = geo.laneMeta.get(key); bg.append(svgEl(svg, 'line', { x1: 112, x2: geo.width - 20, y1: lane.y + 21, y2: lane.y + 21, class: `graph-lane-line route-${key}` })); const t = svgEl(svg, 'text', { x: 16, y: lane.y + 26, class: `graph-lane-title route-${key}` }); t.textContent = lane.label; bg.append(t); }
  svg.append(bg);

  const edgeLayer = svgEl(svg, 'g', { class: 'graph-edges' });
  for (const edge of edges) { const from = geo.positions.get(edge.from), to = geo.positions.get(edge.to); if (!from || !to) continue; edgeLayer.append(svgEl(svg, 'path', { d: edgePath(from, to), class: `graph-edge ${edge.kind}`, 'marker-end': 'url(#graph-arrow)' })); const text = edge.label || (edge.hiddenScenes ? `${edge.hiddenScenes} сцен` : ''); if (text) { const label = svgEl(svg, 'text', { x: (from.x + from.width + to.x) / 2, y: (from.y + from.height / 2 + to.y + to.height / 2) / 2 - 6, class: `graph-edge-label ${edge.label ? 'option' : 'collapsed'}` }); label.textContent = text.length > 36 ? `${text.slice(0, 34)}…` : text; edgeLayer.append(label); } }
  svg.append(edgeLayer);
  const nodes = svgEl(svg, 'g', { class: 'graph-nodes' });
  for (const node of model.nodes) if (visible.has(node.id)) { const pos = geo.positions.get(node.id); if (pos) nodes.append(makeNodeGroup(svg, node, pos, { ...options, matched: matched.has(node.id) })); }
  svg.append(nodes);
...
function renderRoutes(host, model, options) {
  ...
  const routeKeys = ['equal', 'fire', 'mask', 'direct'].filter(key => model.nodes.some(n => n.kind === 'scene' && n.routeKey === key));
